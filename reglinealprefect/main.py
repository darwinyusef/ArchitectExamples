...
from shema_api import (ParametrosCotizacion, RespuestaCotizacion,
                         EstadisticasServicio, SolicitudLote)
...
@task(name="Publicar resultado en Redis Pub/Sub", retries=2, retry_delay_seconds=1)
async def publicar_en_redis(id_cotizacion: str, respuesta: dict):
    """Publica el resultado en Redis pub/sub (si está disponible)."""
    if not redis_client:
        print(f"⚠️  Redis no disponible - skip publicación para {id_cotizacion}")
    ...
    try:
        ...
        print(f"✅ Resultado publicado en Redis para cotización {id_cotizacion}")
    except Exception as e:
        print(f"⚠️  Error publicando en Redis: {e}")
        raise


@task(name="Guardar cotización en cache Redis", retries=2, retry_delay_seconds=1)
async def guardar_en_cache_redis(id_cotizacion: str, respuesta: dict, ttl_segundos: int = 3600):
    """Guarda la cotización en cache de Redis por un tiempo determinado."""
    if not redis_client:
        print(f"⚠️  Redis no disponible - skip cache para {id_cotizacion}")
        return
    ...
    try:
        ...
        print(f"✅ Cotización {id_cotizacion} guardada en cache por {ttl_segundos}s")
    except Exception as e:
        print(f"⚠️  Error guardando en cache Redis: {e}")
        raise


@flow(name="Procesar resultado de cotización", task_runner=ThreadPoolTaskRunner())
async def procesar_resultado_cotizacion_flow(id_cotizacion: str, respuesta: dict):
    """
    Flow de Prefect que orquesta las operaciones post-cotización:
    1. Publicar resultado en canal de Redis (Pub/Sub)
    2. Guardar en cache de Redis
    """
    # Ejecutar tareas en paralelo
    await publicar_en_redis(id_cotizacion, respuesta)
    await guardar_en_cache_redis(id_cotizacion, respuesta)
...
async def crear_cotizacion(
    ...
    Nota: Las operaciones de Redis (pub/sub y cache) son orquestadas por Prefect.
    """
    try:
        # Generar ID único
        ...
        # Procesar resultado usando Prefect Flow (en background)
        respuesta_dict = respuesta.dict()
        background_tasks.add_task(
            procesar_resultado_cotizacion_flow,
            id_cotizacion,
            respuesta_dict
        )
